...
export default function ServicesOverview() {
  return (
    <section className="pt-0 -mt-6 pb-6 bg-background relative z-20">
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, index) => {
            ...
            // Use query if available, otherwise fallback to title, encoded for URL
            const queryParam = encodeURIComponent(service.query || service.title);

            return (
              <Link key={index} href={`/job-finder?q=${queryParam}&focus=true`}>
                <Card className="p-6 hover-elevate active-elevate-2 transition-all cursor-pointer h-full" data-testid={`card-service-${index}`}>
